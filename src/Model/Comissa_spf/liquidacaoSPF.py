...
def Liquidação_comissa_spf(pessoa,flag):
    try :
        ...
        logging.info("REALIZANDO LIQUIDACAO DE COMISSAO SPF")
        ...
        if valorDealer != valor_cliente or valorDealer == valor_cliente:
            valorSobras = (float(valor_cliente) - float(valorDealer)) 
        ...
        valor_finals = glob_imposto_irrf(valorDealer)
...
###################################################     LIQUIDAÇÃO      ###################################################
        ...
        while fechar != None:
            ...
            p.sleep(1)
            fechar = p.locateCenterOnScreen(f'{img}fechar38.png',confidence=0.95)
        return True, valorSobras,valorDealer
    except:
